...
function aplicarPromociones() {
  ...
  for (const productoNombre in inventario) {
    ...
    promotionsData.promociones.forEach(promocion => {
      if (promocion.producto_aplica === productoNombre) {
    ...
    if (mejorPromocion) {
      ...
      let cantidadLlevada = nuevoPrecio; // Ajuste aquí

      producto.precio = nuevoPrecio;
      producto.cantidadKilos = cantidadLlevada * aplicables; // Ajuste aquí
...
function actualizarPromocion(productoNombre) {
  ...
  promotionsData.promociones.forEach(promocion => {
    if (inventario.hasOwnProperty(promocion.producto_aplica)) {
...
function mostrarPromociones(descuentosDiv) {
  ...
  promocionesAplicadas.forEach(promocion => {
    ...
    descuentoElementos.textContent = `${promocion.promocion} de ${promocion.producto}, LLeva: ${promocion.vecesAplicables}`;
